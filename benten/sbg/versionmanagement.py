...
def update_repo_tag(view: YamlView, new_id=None):
    ...
    if "sbg:id" not in view.yaml and new_id is None:
        return
    ...
    new_id = new_id or file_not_pushed_suffix

    base = view.root()
    if base.children:  # Obviously, we are allowing inline views
        new_lines = _propagate_local_edits_tag(
            base.raw_lines, base.yaml, view.inline_path, new_id)
        base.set_raw_text("".join(new_lines))
        base.synchronize_text()
    else:
        new_lines = _add_or_replace_repo_tag(base.raw_lines, base.yaml, new_id)
        base.set_raw_text("".join(new_lines))


def _add_or_replace_repo_tag(lines, obj, new_id):
    if "sbg:id" in obj:
        sbg_id_v = obj["sbg:id"]
...
def _propagate_local_edits_tag(lines, obj, inline_path, new_id):
    if len(inline_path):  # Still a ways to go
        ...
        return _add_or_replace_repo_tag(lines, obj, new_id)
